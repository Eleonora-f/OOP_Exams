...
public class Test {
	 ...
	 * initFactory qui sotto. Realizza una factory per un concetto di corso di
	 * laurea (program),
	 * catturato dall'interfaccia UniversityProgram.
	 * 
	 * Come si può vedere nel metodo fillProgram qui sotto, ad un CDL si
	 * associa innanzitutto un insieme di corsi (chiave della mappa), ognuno con un
	 * certo numero di
	 * crediti e un insieme di gruppi di corsi (come coppia, valore della mappa),
	 * ad esempio:
	 * map.put("AI", new Pair<>(Set.of(OPTIONAL, OPTIONAL_A), 6));
	 * significa che il corso AI ha 6 CFU ed è usabile sia nel gruppo OPTIONAL che
	 * in OPTIONAL_A
	 * 
	 * A quel punto il CDL ha un metodo per stabilire se una certa selezione di
	 * corsi (ossia
	 * un curriculum presentato da uno studente) soddisfa o meno i requisiti,
	 * espressi da una logica
	 * CDL-specifica, in termini di numero di crediti (minimi e/o massimi) per ogni
	 * gruppo. Questa
	 * logica è specificata nella factory. Se ad esempio si dice che gli OPTIONAL
	 * devono essere al
	 * massimo 12 CFU significa che la somma dei CFU dei corsi che posso appartenere
	 * al gruppo
	 ...
	 * punteggio:
	 * 
	 * - implementazione dei quattro metodi della factory (ossia, nella parte
	 * obbligatoria è sufficiente
	 * implementarne 3 a piacimento)
	 * - la buona progettazione della soluzione, utilizzando patterns che portino a
	 * codice succinto
	...
	public void testFlexible() {
		...
		assertFalse(program.isValid(Set.of("PROG_LANGS", "AI", "DEEP_LEARN", "NETWORKS", "THESIS", "SEM_WEB"))); // not
																													// 48
																													// CFU
	}
	...
	public void testFixed() {
		...
		assertFalse(
				program.isValid(Set.of("PROG_LANGS", "TECH_WEB", "ROBOTICS", "DEEP_LEARN", "DATA_MINING", "THESIS"))); // Too
																														// much
																														// "mandatory"
		assertFalse(program.isValid(Set.of("PROG_LANGS", "VISION", "ROBOTICS", "DEEP_LEARN", "DATA_MINING", "THESIS"))); // No
																															// Free
																															// course
																															// allowed
	}
	...
	public void testBalanced() {
		...
		assertFalse(program.isValid(Set.of("PROG_LANGS", "SOFT_ENG", "DEEP_LEARN", "ROBOTICS", "VISION", "THESIS"))); // Thesis+Free
																														// >
																														// 12
		assertFalse(program.isValid(Set.of("PROG_LANGS", "AI", "ROBOTICS", "DEEP_LEARN", "DATA_MINING", "THESIS"))); // not
																														// enough
																														// mandatory
	}
	...
	public void testStructured() {
		...
		fillProgram(program);
		assertTrue(
				program.isValid(Set.of("PROG_LANGS", "SMART_CITY", "DATA_MINING", "DEEP_LEARN", "NETWORKS", "THESIS"))); // ok
		assertFalse(
				program.isValid(Set.of("PROG_LANGS", "SEM_WEB", "DATA_MINING", "BUS_ANALYTICS", "NETWORKS", "THESIS"))); // not
																															// ok
	}
}
