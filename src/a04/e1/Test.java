...
/**
 * Si consulti la documentazione della interfaccia BankAccountFactory, che
 * modella
 * una factory per BankAccount, che a sua volta modella un conto corrente con un
 * certo saldo (balance),
 * e possibilità di deposito soldi (deposit) e prelievo (withdrawal):
 * dipendentemente dall'implementazione
 * il prelievo potrebbe essere soggetto a tasse, blocchi, o credito, o
...
public class Test {
	...
	public void testWithFee() {
		...
		// quello che prelievo, se disponible, viene rimosso, togliendo un euro in più
		// se prelievo più di 35
		assertEquals(0, account.getBalance());
		account.deposit(100);
		account.deposit(30);
		assertEquals(130, account.getBalance());
	...
	public void testWithCredit() {
		...
		// il conto può andare in "rosso" fino a 99 euro, ma ogni operazione che mi
		// manda in rosso viene tassata di 10 euro
		assertEquals(0, account.getBalance());
		account.deposit(100);
		account.deposit(30);
		assertEquals(130, account.getBalance());
	...
	public void testWithBlock() {
		...
		// se si tenta di prelevare più di quello che si ha il conto si blocca e non si
		// può più prelevare nulla
		assertEquals(0, account.getBalance());
		account.deposit(100);
		account.deposit(30);
		assertEquals(130, account.getBalance());
	...
	public void testWithFeeAndCredit() {
		...
		// tassa fissa 1 euro a prelievo, credito fino a max 100 euro esclusi, e 10 euro
		// di tassa quando si va in rosso
